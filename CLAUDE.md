...
## 共有データベース
- App B（予約・来院管理）とApp A（在庫・技工物管理）は同じSupabase DBを共有している
- App Aのリポジトリは別だが、同じSupabaseプロジェクトに接続している
- lab_orders、patients、users、labs などのテーブルは両アプリから参照される
- lab_ordersのステータスは：未発注 / 製作中 / 納品済み / セット完了 / キャンセル
- App Aでテーブル構造が変更された場合、App B側のクエリも更新が必要
- App Bの独自テーブル：patients、appointments
